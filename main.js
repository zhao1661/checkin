const glados = async () => {
  ...
  for (const cookie of String(process.env.GLADOS).split('\n')) {
    ...
    try {
      const common = {
        'cookie': cookie,
        'referer': 'https://glados.clouds/console/checkin',
        'user-agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.0)',
      }
      const action = await fetch('https://glados.clouds/api/user/checkin', {
        method: 'POST',
        headers: { ...common, 'content-type': 'application/json' },
        body: '{"token":"glados.one"}',
      }).then((r) => r.json())
      if (action?.code) throw new Error(action?.message)
      const status = await fetch('https://glados.clouds/api/user/status', {
        method: 'GET',
        headers: { ...common },
      }).then((r) => r.json())
